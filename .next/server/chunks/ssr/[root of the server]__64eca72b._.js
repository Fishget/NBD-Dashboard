...
if (typeof rawPrivateKeyFromEnv === 'string' && rawPrivateKeyFromEnv.trim() !== '') {
    ...
    if (unescapedKey.startsWith('-----BEGIN PRIVATE KEY-----') && unescapedKey.endsWith('-----END PRIVATE KEY-----') && unescapedKey.includes('\n') // Ensure there are actual newlines within the key block
    ) {
        const coreKeyContent = unescapedKey.substring('-----BEGIN PRIVATE KEY-----'.length, unescapedKey.length - '-----END PRIVATE KEY-----'.length).trim();
        ...
        // console.log('Successfully processed GOOGLE_PRIVATE_KEY.');
            ...
            console.warn('GOOGLE_PRIVATE_KEY warning: Processed key has PEM markers but no content in between. ' + 'Original GOOGLE_PRIVATE_KEY (first 30 chars, trimmed): "' + (rawPrivateKeyFromEnv || "").trim().substring(0, 30) + '". ' + 'Sheet operations will likely fail.');
            PRIVATE_KEY = undefined;
        }
    } else {
        console.warn('GOOGLE_PRIVATE_KEY warning: Malformed structure after processing. ' + `Key starts with (after processing and trimming): "${unescapedKey.substring(0, 30)}...". ` + `Key ends with (after processing and trimming): "...${unescapedKey.substring(Math.max(0, unescapedKey.length - 30))}". ` + `Includes newlines: ${unescapedKey.includes('\n')}. ` + 'Original GOOGLE_PRIVATE_KEY (first 30 chars, trimmed): "' + (rawPrivateKeyFromEnv || "").trim().substring(0, 30) + '". ' + 'Ensure it is a valid PEM key with actual newlines between markers, correctly escaped newlines (\\n) if stored as a single line string in .env, and no extraneous characters.');
        PRIVATE_KEY = undefined;
    }
} else {
...
async function /*#__TURBOPACK_DISABLE_EXPORT_MERGING__*/ getSheetsClient() {
    if (!SHEET_ID || !SERVICE_ACCOUNT_EMAIL || !PRIVATE_KEY) {
        const errorParts = [
            'Cannot initialize Sheets client due to missing/invalid credentials:'
        ];
    ...
    try {
        ...
        if (error instanceof Error && (error.message.includes('DECODER routines') || error.message.includes('PEM routines') || error.message.includes('private key') || error.message.includes('asn1 encoding'))) {
            console.error('This error during auth initialization often indicates an issue with the GOOGLE_PRIVATE_KEY format or value even after initial checks. Ensure it is a valid PEM-formatted private key, with actual newlines if copied from a source that used \\n. The key might have passed basic structural checks but is still not parsable by the crypto library.');
        }
        return null;
    }
}
...
async function /*#__TURBOPACK_DISABLE_EXPORT_MERGING__*/ submitDataAction(prevState, formData) {
    try {
        ...
        const validatedFields = __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$validators$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["sheetRowSchema"].safeParse(rawData);
        ...
        const dataToAppend = validatedFields.data;
        ...
        if (success) {
            (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$cache$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["revalidatePath"])('/'); // Revalidate home page to show new data
            (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$cache$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["revalidatePath"])('/admin'); // Revalidate admin page if it displays data
            return {
                success: true,
...
async function /*#__TURBOPACK_DISABLE_EXPORT_MERGING__*/ loginAction(prevState, formData) {
    try {
        const rawData = Object.fromEntries(formData.entries());
        const validatedFields = __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$validators$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["loginSchema"].safeParse(rawData);
        if (!validatedFields.success) {
            return {
                success: false,
...
async function /*#__TURBOPACK_DISABLE_EXPORT_MERGING__*/ saveSheetConfigAction(prevState, formData) {
    try {
        const rawData = Object.fromEntries(formData.entries());
        const validatedFields = __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$validators$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["sheetConfigSchema"].safeParse(rawData);
        if (!validatedFields.success) {
            return {
                success: false,
...
//# sourceMappingURL=%5Broot%20of%20the%20server%5D__64eca72b._.js.map